...
# makes a molecule by appending several attachments to a parent atom and joining meshes
...
def makeMolecule(name, parent_name, attachments, location = bpy.context.scene.cursor.location):
    ...
    parent_obj = parent_ref_obj.copy()
    parent_obj.data = parent_ref_obj.data.copy()
    parent_obj.location = location
    parent_obj.name = name
    bpy.data.collections["Molecules"].objects.link(parent_obj)
...
if len(bpy.data.collections["Molecules"].all_objects) == 0:
